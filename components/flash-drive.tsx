...
/** Голый анодированный алюминий — цвет проушины под кольцо. */
const МЕТАЛЛ = "#b8babc";
      ...
      >
        ...
        <g transform={зеркало}>
          ...
          {/* Анодировка: цвет под шлифовкой, снимок ложится умножением.
              Дальний торец не окрашивается — там проушина под заводное
              кольцо, это голый металл. Цвет обрывается ровно на дуге,
              поэтому граница совпадает с началом скругления. */}
          <rect
            x={ПЛАСТИНА_X}
            y={ПЛАСТИНА_Y}
            width={SPEC.plate - КРАЙ}
            height={SPEC.plateH}
            fill={tint(color)}
          />
          <rect
            x={ПЛАСТИНА_X + SPEC.plate - КРАЙ}
            y={ПЛАСТИНА_Y}
            width={КРАЙ}
            height={SPEC.plateH}
            fill={tint(МЕТАЛЛ)}
