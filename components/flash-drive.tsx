...
const ZONE1_R = ZONE2_L - SPEC.gap;

/**
 * Левая граница надписи — кромка колпачка, а не край поля гравировки.
 *
 * По чертежу поле начинается в 2,5 мм от торца корпуса, но чёрный
 * колпачок закрывает первые 5,4 мм: строка в полную ширину заезжала
 * на чёрное, и гравировки там нет и быть не может. На фотографии
 * партии надпись тоже начинается сразу за колпачком.
 *
 * Правый край зон не двигаем — он от чертежа. Двигается только левый,
 * и зона на эти 2,9 мм короче: со знаком 29,1 мм вместо 32, без знака
 * 42,1 вместо 45. Сжатие считает по фактической зоне, поэтому длинная
 * строка просто дожимается сильнее.
 */
const ЛЕВЫЙ_КРАЙ = Math.max(FIELD_L, КОЛПАЧОК);
const ZONE1_W = ZONE1_R - ЛЕВЫЙ_КРАЙ;
const ZONE1_MID = (ЛЕВЫЙ_КРАЙ + ZONE1_R) / 2;
/** без знака надпись занимает всё поле гравировки — от колпачка до края */
const WIDE_W = FIELD_R - ЛЕВЫЙ_КРАЙ;
const WIDE_MID = (ЛЕВЫЙ_КРАЙ + FIELD_R) / 2;
...
export function FlashDrive({
  ...
  useEffect(() => {
    ...
    const зона =
      side === "back" ? SPEC.backField : apparatusId ? ZONE1_W : WIDE_W;
    setSqueeze(Math.min(SQUEEZE, зона / w));
  }, [key, side, apparatusId]);
