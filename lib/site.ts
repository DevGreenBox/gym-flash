 ...
 *
 * Заказчик требует Segoe Print, сжатый по горизонтали на 50 %. Сам Segoe
 * Print на сайт поставить нельзя: он идёт в комплекте с Windows, и его
 * лицензия запрещает раздачу файла — а веб-шрифт это ровно раздача файла.
 * Отдельная веб-лицензия у Monotype существует и стоит денег; пока её нет,
 * стоит Neucha — ближайший по рисунку кириллический почерк: тоже прямой,
 * печатный, буквы не связаны. Сравнение с настоящей гравировкой лежит
 * в docs/шрифт-гравировки.md.
 *
 * TODO(client): купить веб-лицензию Segoe Print — и заменить здесь одну
 * строку, остальное считается само.
 *
 * `squeeze` — то самое сжатие на 50 %.
 * `em` — средняя ширина знака в долях кегля, по ней ужимается подпись
 * предмета в поле знака.
 */
export const FONTS = [
  {
    id: "print",
