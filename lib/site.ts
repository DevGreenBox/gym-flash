...
/**
 * Гарнитура гравировки — одна, и одинаковая на всех устройствах.
 *
 * Segoe Print с сайта убран. Дело не только в лицензии: он стоит лишь
 * на Windows, и флешка выглядела бы по-разному в зависимости от того,
 * с чего человек зашёл. Конструктор показывает будущую покупку — она
 * не должна меняться от устройства. В цехе при гравировке оригинал
 * никуда не делся, там он и нужен.
 *
 * Playpen Sans — ближайший по скелету из всего, что имеет кириллицу
 * и настоящий жирный: во всём каталоге Google Fonts рукописных
 * с кириллицей пятнадцать, и почти у всех один вес. Буквы прямые,
 * не связаны, той же ширины, что на гравировке; цифры почти один
 * в один. Сравнение с фотографией партии — docs/шрифт-гравировки.md.
 *
 * Вес 400. Чертёж требует жирное начертание, но у Playpen Sans штрих
 * сам по себе толще, чем у Segoe Print, и всё от 500 и выше на пластине
 * выходило плотнее гравировки. Гарнитура переменная, поэтому ступень
 * подобрана сверкой с фотографией партии, а не взята из чертежа.
 */
export const FONTS = [
  {
    id: "print",
    label: "Playpen Sans",
    note: "рукописный, как на гравировке",
    css: "var(--font-playpen), cursive",
    weight: 400,
    em: 0.5,
  },
];
...
/**
 * Предел сжатия гравировки по горизонтали.
 *
 * Чертёж требовал жать всё до 50 %, но на готовой флешке надпись вдвое
 * с лишним шире: «Чиганашина» на изделии 25,1 мм, а при 50 % выходило
 * 9,5. Заказчик решил в пользу изделия — постоянного сжатия больше нет.
 *
 * Единица не значит «не сжимать никогда»: строка, не влезающая в свою
 * зону, по-прежнему дожимается ровно настолько, чтобы уложиться.
 * Кегль при этом не трогается — три строки обязаны стоять вровень.
 */
export const SQUEEZE = 1;
