...
export const site = {
  ...
  /** Ссылка из приложения: «Профиль → Поделиться». */
  max: "https://max.ru/u/f9LHodD0cOLqPLLyiUS_9BftDzK1jedwgdVfKNpfVzGpUKEbLf0sOmpTUDA",
};
...
export const SPEC = {
  ...
  backField: 28,
  /** предел строки оборота — тот же принцип, зона своя */
  backChars: 20,
  lines: 3,
  /**
   * Предел строки — не из чертежа: там ограничена зона, а не число знаков.
   * Держим его широким, чтобы примеры заказчика влезали дословно
   * («г. Ростов-на-Дону» — семнадцать знаков), а за ширину отвечало
   * сжатие: чертёж прямо велит дожимать шрифт до 32 мм.
   */
  charsPerLine: 20,
};
