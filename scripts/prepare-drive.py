...
Силуэт корпуса рисуется вектором (`components/drive-shape.tsx`): на чертеже
заказчика у корпуса прямые верх и низ и эллиптические торцы, и обрезкой
снимка такую форму не собрать. От снимка нужна только шлифовка —
`public/drive/plate.png`, прямоугольник пластины, который в модели
ложится на цвет умножением.
...
def save_plate(fixed, x0, x1, top, bot, scale):
    """
    Шлифовка для модели — только ровная середина пластины.

    Снимок сделан с настоящей флешки, и у него запечён собственный торец:
    светлый кант, тёмный шов и вырез под кольцо. Модель растягивает файл
    на свои 50 мм, и этот запечённый торец попадал внутрь корпуса —
    получалась светлая полоса поперёк и тёмное пятно у края. Оттенки
    «съезжали»: свет лежал там, где его на предмете нет.

    Поэтому берём середину — по 2 мм от каждого края долой — и убираем
    из неё продольный завал яркости (185 слева против 172 справа): он
    красил один конец корпуса темнее другого. Убирается только плавный
    завал, шлифовальные штрихи остаются — тренд считается по столбцам
    и сглаживается окном в 60 px.

    Поперечная растушёвка (сверху светлее, снизу темнее) — настоящая
    тень скруглённого корпуса, её не трогаем.
    """
    import numpy as np

    m = round(2 * scale)  # 2 мм запаса от каждого края
    crop = fixed.crop((x0 + m, top + m, x1 - m, bot - m)).convert("RGB")
    a = np.asarray(crop).astype(float)

    trend = a.mean(axis=(0, 2))
    k = min(60, len(trend) // 2 * 2 + 1)
    pad = np.pad(trend, k // 2, mode="edge")
    smooth = np.convolve(pad, np.ones(k) / k, mode="valid")[: len(trend)]
    a *= (trend.mean() / smooth)[None, :, None]

    out = Image.fromarray(np.clip(a, 0, 255).astype("uint8"))
    out.save(OUT / "plate.png")
    print(f"шлифовка: {out.size[0]}×{out.size[1]} px, завал по длине снят")
...
def main():
    ...
    save_plate(fixed, minx + cap_target, plate_end - shift, top, bot, scale)
